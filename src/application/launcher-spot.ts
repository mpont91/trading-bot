...
export class LauncherSpot {
  ...
  async start(): Promise<void> {
    logger.info('Launcher spot is about to start for the first time.')
    await this.report()
    await this.execute()
  ...
  private async execute(): Promise<void> {
    logger.info('Launcher spot is going to start the execution managers.')
    try {
  ...
  private async report(): Promise<void> {
    logger.info('Launcher spot is going to start the reporting managers.')
    try {
      console.log('TODO: implement them')
    } catch (error: unknown) {
